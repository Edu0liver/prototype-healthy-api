...
// New builds a Cipher from the configured 32-byte hex key. When no key is set
// (local dev), encryption is disabled and a passthrough cipher is returned.
func New(cfg *config.Config) (*Cipher, error) {
	key := cfg.Crypto.EncryptionKey
	if key == "" {
