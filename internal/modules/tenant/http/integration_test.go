...
// TestBrandingByHost covers the public GET /branding/host?host= lookup.
func TestBrandingByHost(t *testing.T) {
	db := testsupport.NewPostgres(t)
	r, adminTok, _ := newRouter(t, db)

	// Unknown host must return 404.
	w := do(t, r, http.MethodGet, "/branding/host?host=unknown.no.such.host", nil, "")
	...
	// GET /branding/host?host=brand.acme.test → must return the tenant's branding.
	w = do(t, r, http.MethodGet, "/branding/host?host=brand.acme.test", nil, "")
