...
func New(db *database.DB, rdb *redisx.Client, conv *convsvc.Service, repo *repositories.Repository, cfg *config.Config, log *zap.Logger) *Service {
...
func (s *Service) audit(ctx context.Context, res repositories.Resolved, event string, env envelope) {
...
func (s *Service) handleMessage(ctx context.Context, res repositories.Resolved, env envelope) error {
	var d messageData
...
func (s *Service) handleConnection(ctx context.Context, res repositories.Resolved, env envelope) error {
...
func (s *Service) handleStatus(ctx context.Context, res repositories.Resolved, env envelope) error {
	var d statusData
	if err := json.Unmarshal(env.Data, &d); err != nil {
		return err
	}
